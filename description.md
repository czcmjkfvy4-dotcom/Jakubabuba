...
The bundled BILLADENC4 runtime enforces one final dragon per month, activates Ruby Golem round growth, enables fixed-slot autosaves every turn, enables the `nwcbilladen` creator code and preserves fitted level 8 recruitment animations. Version 1.5.7 also fixes PNG petrification. The mod contains creator tools and a native eight-scenario campaign with Polish in-game stories, unit statistics, prices and progressive recruitment locks. Dragon Citadel will continue to be developed and updated on GitHub.
...
Dołączony runtime BILLADENC4 wymusza pojawienie się jednego ostatniego smoka raz na miesiąc, uruchamia wzrost Rubinowego Golema, nadpisywany autosave co turę i kod twórcy `nwcbilladen`. Wersja 1.5.7 poprawia wygląd skamienienia jednostek PNG. Mod zawiera także własną scenę oblężenia ze smoczymi wieżyczkami, narzędzia twórcy oraz osiem map kampanii z polską historią, statystykami, kosztami i stopniowym odblokowywaniem jednostek. Projekt będzie dalej rozwijany i aktualizowany na GitHubie.
